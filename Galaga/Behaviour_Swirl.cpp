...
dae::MovementActionSequenceHandle dae::SwirlBehaviour::CreateInstance(Enemy& enemyCreator)
{
	...
	swirlBehaviour.AddActionFunction([enemy, grid, endRelativePos, swirlData](float, const MovementData&) mutable
		{
			...
			if (glm::length(delta) > swirlData.PostSwirlRadiusToAttach)
			{
				...
				delta = normDelta * swirlData.PostSwirlSpeedToReturn * GameTime::GetInstance().GetElapsedSeconds();
