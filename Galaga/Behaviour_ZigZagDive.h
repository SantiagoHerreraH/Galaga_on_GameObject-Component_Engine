...
namespace dae {
	...
	MovementActionSequence AddZigZagDiveBehavior(Scene& scene, GameObjectHandle enemy, GameObjectHandle grid, GameObjectHandle player, const glm::vec3& relativePos, std::shared_ptr<Event<GameObject&>> onEndAction, bool goTowardsLeft) {
		...
		zigZagDiveBehavior.AddEndSubAction([onEndAction, enemy]() mutable {
