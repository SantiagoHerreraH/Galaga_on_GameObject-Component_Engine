...
namespace dae {
	...
	GameObjectHandle CreateBirdEnemy(
		...
		bool goTowardsLeft) 
	{
		...
		transformData.Position = relativePos;
		transformData.Scale = { 1, 1, 1 };
		//transformData.Rotation.z = 180;

		//------

		SpriteSheet spriteSheet{ dae::CTextureHandle{"boss.png"}, 1, 2 };
		...
		IntStat pointsInFormation{ 150, 150, 150 };
		...
		IntStat pointsWhileDiving{ 400, 400, 400 };
		...
		onAct.Subscribe([behaviours, chosenMovementAction, enemy, pointsWhileDiving]() mutable {
			...
			if (!behaviours[chosenIndex].RestartSequence())
			{
				chosenIndex = 0;
				behaviours[chosenIndex].RestartSequence();
			}

			(*chosenMovementAction) = behaviours[chosenIndex];
